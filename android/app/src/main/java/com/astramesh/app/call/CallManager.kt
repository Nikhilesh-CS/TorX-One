...
class CallManager(
    ...
    fun startAudioCall(peerKey: String) {
        scope.launch {
            ...
            if (routeContext.transport == com.astramesh.app.network.Transport.FAILED) {
    ...
    fun handleSignal(packetType: String, rawPayload: String, senderKey: String) {
        scope.launch {
            runCatching {
                ...
                when (packetType) {
                    com.astramesh.app.network.MeshProtocol.TYPE_CALL_OFFER -> handleOffer(signal, senderKey)
                    com.astramesh.app.network.MeshProtocol.TYPE_CALL_ANSWER -> handleAnswer(signal, senderKey)
                    com.astramesh.app.network.MeshProtocol.TYPE_ICE_CANDIDATE -> handleIce(signal)
                }
            }.onFailure { e ->
    ...
    private suspend fun handleOffer(signal: CallSignal, senderKey: String) {
        ...
        val peerName = contact?.name ?: "Astra contact"
    ...
    private suspend fun handleAnswer(signal: CallSignal, senderKey: String) {
        ...
        stateStore.update(CallUiState.Connecting(callId, senderKey, contact?.name ?: "Astra contact", signal.mode))
    }
