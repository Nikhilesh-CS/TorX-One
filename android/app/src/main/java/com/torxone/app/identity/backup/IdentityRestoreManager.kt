    ...
    suspend fun restoreBackup(inputStream: InputStream, password: CharArray): Result<Unit> = withContext(Dispatchers.IO) {
        try {
            ...
            if (dto.schemaVersion > 1) {
                return@withContext Result.failure(Exception("Backup schema v${dto.schemaVersion} is not supported by this version of TorX One."))
            }
            ...
            val service = TorXOneService.getInstance()
            ...
            try {
                ...
                val db = androidx.room.Room.databaseBuilder(
                    ...
                    com.torxone.app.data.AppDatabase::class.java,
                    "astra-mesh-db"
                ).build()
                ...
                val restoredProfile = com.torxone.app.data.ProfileEntity(
                    ownerKey = "LOCAL_USER",
                    name = identity.name,
