                ...
                when (intent.action) {
                    ACTION_REPLY -> {
                        ...
                        if (!replyText.isNullOrBlank()) {
                            Log.d("NotificationAction", "Sending quick reply to $contactKey ($conversationType)")
                            if (conversationType == "group") {
                                router.sendGroupMessage(contactKey, replyText)
                            } else {
                    ...
                    ACTION_MARK_READ -> {
                        Log.d("NotificationAction", "Marking messages as read for $contactKey ($conversationType)")
                        db.messageDao().markMessagesAsRead(contactKey, conversationType)
                        NotificationHelper.clearContactNotifications(context, contactKey)
                    }
                    ACTION_MUTE -> {
                        val durationMs = intent.getLongExtra("durationMs", 0L)
                        Log.d("NotificationAction", "Muting $contactKey ($conversationType) for $durationMs ms")
                        val muteUntil = if (durationMs == -1L) -1L else System.currentTimeMillis() + durationMs
                        if (conversationType == "group") {
                            val group = db.groupDao().getGroup(contactKey)
                            if (group != null) {
