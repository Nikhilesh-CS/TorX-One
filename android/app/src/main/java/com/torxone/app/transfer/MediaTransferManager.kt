...
import com.torxone.app.data.MediaTransferEntity
...
import com.torxone.app.realtime.AstraFastLane
import com.torxone.app.realtime.RealtimeSendResult
...
class MediaTransferManager(
    ...
    private suspend fun sendQueuedTransfer(messageId: String, forceWifiDirect: Boolean) = withContext(Dispatchers.IO) {
        ...
        repeat(4) { attempt ->
            val result = messageRouter.sendRawPayload(message.contactKey, metadataPayload, com.torxone.app.network.MeshProtocol.TYPE_MEDIA_OFFER)
            if (!result.success) {
        ...
        RandomAccessFile(file, "r").use { raf ->
            ...
            while (contiguousAcked < totalChunks - 1 && !failed) {
                ...
                launch(Dispatchers.Default) {
                    ...
                    val result = messageRouter.sendRawPayload(message.contactKey, payload, com.torxone.app.network.MeshProtocol.TYPE_MEDIA_CHUNK)
                    if (!result.success) {
    ...
    private suspend fun tryFastLaneTransfer(contact: com.torxone.app.data.ContactEntity, file: File): RealtimeSendResult {
    ...
    fun handleMediaPacket(packetType: String, jsonString: String, senderKey: String) {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                ...
                when (packetType) {
                    com.torxone.app.network.MeshProtocol.TYPE_MEDIA_OFFER -> handleMediaOffer(json, senderKey)
                    com.torxone.app.network.MeshProtocol.TYPE_MEDIA_CHUNK -> handleMediaChunk(json, senderKey)
                    com.torxone.app.network.MeshProtocol.TYPE_MEDIA_ACK -> handleMediaAck(json)
                    com.torxone.app.network.MeshProtocol.TYPE_MEDIA_COMPLETE -> handleMediaComplete(json)
                }
            } catch (e: Exception) {
    ...
    private fun sendMediaAck(messageId: String, chunkIndex: Int, senderKey: String) {
        ...
        CoroutineScope(Dispatchers.IO).launch {
            try {
                messageRouter.sendRawPayload(senderKey, payload, com.torxone.app.network.MeshProtocol.TYPE_MEDIA_ACK)
            } catch (e: Exception) {
    ...
    private suspend fun verifyAndCompleteTransfer(messageId: String, msg: MessageEntity, tempFile: File, senderKey: String) {
        ...
        if (msg.checksum == null || finalChecksum == msg.checksum) {
            ...
            messageRouter.sendRawPayload(senderKey, payload, com.torxone.app.network.MeshProtocol.TYPE_MEDIA_COMPLETE)
